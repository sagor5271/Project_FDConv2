...
@BACKBONES.register_module('ResNet_FDConv')
class ResNet(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 need_grad_list=['attention'],
                 init_cfg=None):
        super(ResNet, self).__init__(init_cfg)
        self.need_grad_list = need_grad_list
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            res_layer = self.make_res_layer(
                # self.block if (i + 1) > self.frozen_stages else Bottleneck ###
                block=self.block, 
                # inplanes=self.inplanes,
                inplanes=inplanes, ###
                planes=planes,
    ...
    def _freeze_stages(self):
        ...
        for i in range(1, self.frozen_stages + 1):
            ...
            m.eval()
            for param in m.parameters():
                whitelist_parameters_requiring_grad(m, self.need_grad_list)
                # param.requires_grad = False
    ...
    def forward(self, x): ###
        ...
        if self.deep_stem:
            ...
            x = checkpoint(self.stem, x) if (self.use_checkpoint and x.requires_grad) else self.stem(x)
        else:
            x = self.conv1(x)
            x = self.norm1(x)
            x = self.relu(x)
